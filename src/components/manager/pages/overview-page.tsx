...
// @ts-ignore
import { tinhDoanhThuThang, countHoaDonChuaThanhToanTrongThang } from "@/services/hoa-don.service"
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                const totalTenants = Array.from(new Set((hopDongList || []).map((hd: any) => hd.khach_thue_id || hd.khach_thue?.id))).length
                ...
                const now = new Date()
                const monthlyRevenue = await tinhDoanhThuThang(selectedHostel.id, now.getFullYear(), now.getMonth() + 1)

                // Hóa đơn chưa thanh toán trong tháng
                const pendingPayments = await countHoaDonChuaThanhToanTrongThang(selectedHostel.id, now.getFullYear(), now.getMonth() + 1)
                        ...
                        <div className="space-y-3">
                            {recentNotifications.length === 0 ? (
                                ...
                                recentNotifications.map((tb: any) => (
                                    <div key={tb.id} className={`flex items-start space-x-3 p-3 rounded-lg border ${tb.trang_thai === 'chua_xu_ly' ? 'bg-yellow-50 border-yellow-200' : 'bg-green-50 border-green-200'}`}>
                                        ...
                                        <div className="flex-1">
                                            <p className="text-sm font-medium text-gray-800">{tb.tieu_de}</p>
                                            <p className="text-xs text-gray-500 mt-1">Phòng {tb.can_ho?.so_can || 'N/A'} • {new Date(tb.ngay_tao).toLocaleDateString('vi-VN')}</p>
                                        </div>
                                    </div>
                                ))
                            )}
