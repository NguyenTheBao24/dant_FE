...
export function InvoicesPage({ invoiceData, userContracts }: InvoicesPageProps) {
    ...
    useEffect(() => {
        // Tìm hợp đồng hiệu lực để load hóa đơn
        const activeContract = userContracts?.find(contract => contract.trang_thai === 'hieu_luc')
        if (activeContract?.id) {
            loadAllInvoices(activeContract.id)
        }
    }, [userContracts])

    const loadAllInvoices = async (hopDongId: number) => {
        setIsLoading(true)
        try {
    ...
    const groupInvoicesByMonth = () => {
        ...
        return Object.values(grouped).sort((a: any, b: any) => b.monthKey.localeCompare(a.monthKey))
    }
            ...
            <div className="space-y-6">
                ...
                <Card>
                    <CardContent className="p-12 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-gray-600 mb-2">Không có thông tin hóa đơn</h3>
                        <p className="text-gray-500">Không tìm thấy hợp đồng hoặc hóa đơn liên quan</p>
                    </CardContent>
                </Card>
            </div>
        )
    ...
    return (
        <>
            <div className="space-y-6">
                ...
                <div className="grid gap-6 md:grid-cols-4">
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-red-700">
                                        {allInvoices.filter(inv => inv.trang_thai === 'chua_tt').length}
                                    </p>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-green-700">
                                        {allInvoices.filter(inv => inv.trang_thai === 'da_thanh_toan').length}
                                    </p>
                    ...
                    <Card className="border-blue-200">
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-sm font-medium text-blue-600">Tổng đã thanh toán</p>
                                    ...
                                    <p className="text-xs text-blue-600 mt-1">
                                        {allInvoices.filter(inv => inv.trang_thai === 'da_thanh_toan').length} hóa đơn
                                    </p>
                                </div>
                ...
                {(() => {
                    const unpaidInvoice = allInvoices.find(inv => inv.trang_thai === 'chua_tt')
                                ...
                                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                                    <div className="flex items-center justify-between mb-4">
                                        <div>
                                            <h4 className="font-semibold text-red-800">
                                                Hóa đơn tháng {new Date(unpaidInvoice.ngay_tao).toLocaleDateString('vi-VN')}
                                            </h4>
                                            <p className="text-sm text-red-600">
                                                Ngày tạo: {new Date(unpaidInvoice.ngay_tao).toLocaleDateString('vi-VN')}
                                            </p>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        <div className="text-center">
                                            ...
                                            <p className="text-red-600">{unpaidInvoice.so_dien_cu || 0}</p>
                                        </div>
                                        <div className="text-center">
                                            <p className="font-medium text-gray-700">Điện mới</p>
                                            <p className="text-red-600">{unpaidInvoice.so_dien_moi || 0}</p>
                                        </div>
                                        <div className="text-center">
                                            <p className="font-medium text-gray-700">Nước cũ</p>
                                            <p className="text-red-600">{unpaidInvoice.so_nuoc_cu || 0}</p>
                                        </div>
                                        <div className="text-center">
                                            <p className="font-medium text-gray-700">Nước mới</p>
                                            <p className="text-red-600">{unpaidInvoice.so_nuoc_moi || 0}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                            <div className="text-center">
                                                ...
                                                <p className="text-red-600">
                                                    {((unpaidInvoice.so_dien_moi || 0) - (unpaidInvoice.so_dien_cu || 0)) * (unpaidInvoice.gia_dien || 0)}₫
                                                </p>
                                            </div>
                                            <div className="text-center">
                                                <p className="font-medium text-gray-700">Tiền nước</p>
                                                <p className="text-red-600">
                                                    {((unpaidInvoice.so_nuoc_moi || 0) - (unpaidInvoice.so_nuoc_cu || 0)) * (unpaidInvoice.gia_nuoc || 0)}₫
                                                </p>
                                            </div>
                                            <div className="text-center">
                                                <p className="font-medium text-gray-700">Dịch vụ</p>
                                                <p className="text-red-600">
                                                    {(unpaidInvoice.gia_dich_vu || 0).toLocaleString('vi-VN')}₫
                                                </p>
                                            </div>
                                            <div className="text-center">
                                                <p className="font-medium text-gray-700">Tiền phòng</p>
                                                <p className="text-red-600">
                                                    {Math.max(0, (unpaidInvoice.so_tien || 0) -
                                                        ((unpaidInvoice.so_dien_moi || 0) - (unpaidInvoice.so_dien_cu || 0)) * (unpaidInvoice.gia_dien || 0) -
                                                        ((unpaidInvoice.so_nuoc_moi || 0) - (unpaidInvoice.so_nuoc_cu || 0)) * (unpaidInvoice.gia_nuoc || 0) -
                    ...
                    <CardContent>
                        {isLoading ? (
                            ...
                            <div className="text-center py-8">
                                ...
                                <p className="text-gray-500">Không có hóa đơn nào được tạo cho hợp đồng này</p>
                            </div>
                        ) : (
                            <div className="space-y-6">
                                {groupInvoicesByMonth().map((monthData: any) => (
                                    <div key={monthData.monthKey} className="border border-gray-200 rounded-lg">
                                        {/* Month Header */}
                                        <div
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center">
                                                    ...
                                                    <div>
                                                        <h3 className="font-semibold text-gray-900">{monthData.monthName}</h3>
                                                        <p className="text-sm text-gray-600">
                                                            {monthData.invoices.length} hóa đơn •
                                                            Tổng: {monthData.totalAmount.toLocaleString('vi-VN')}₫
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center space-x-4 text-sm">
                                                    <div className="text-center">
                                                        <p className="text-green-600 font-medium">{monthData.paidCount}</p>
                                                        <p className="text-green-600">Đã thanh toán</p>
                                                    </div>
                                                    <div className="text-center">
                                                        <p className="text-red-600 font-medium">{monthData.unpaidCount}</p>
                                        ...
                                        {expandedMonths.has(monthData.monthKey) && (
                                            <div className="p-4 space-y-3">
                                                {monthData.invoices.map((invoice: any) => {
                                                    ...
                                                    const soKwh = (invoice.so_dien_moi || 0) - (invoice.so_dien_cu || 0)
                                                    const soM3 = (invoice.so_nuoc_moi || 0) - (invoice.so_nuoc_cu || 0)
                                                    const tienDien = soKwh * (invoice.gia_dien || 0)
                                                    const tienNuoc = soM3 * (invoice.gia_nuoc || 0)
                                                    const tienDichVu = invoice.gia_dich_vu || 0
                                                    const tienPhong = (invoice.so_tien || 0) - tienDien - tienNuoc - tienDichVu
                                                    ...
                                                    return (
                                                        <div
                                                            key={invoice.id}
                                                            className={`border rounded-lg p-4 hover:shadow-md transition-shadow ${invoice.trang_thai === 'da_thanh_toan'
                                                                ...
                                                                <div className="flex items-center space-x-4">
                                                                    <div className="flex-1">
                                                                        <h3 className="font-semibold text-gray-900">
                                                                            Hóa đơn {new Date(invoice.ngay_tao).toLocaleDateString('vi-VN', { month: 'long', year: 'numeric' })}
                                                                        </h3>
                                                                        <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
                                                                            ...
                                                                            <div className="flex items-center">
                                                                                ...
                                                                                <span className="font-medium">
                                                                                    {(invoice.so_tien || 0).toLocaleString('vi-VN')}₫
                                                                                </span>
                                                                            </div>
                                                                        ...
                                                                        {breakdown && (
                                                                            <div className="mt-2 flex items-center space-x-4 text-xs">
                                                                                <div className="flex items-center text-blue-600">
                                                                                    ...
                                                                                    <span>Phòng: {formatCurrency(breakdown.tienPhong || 0)}</span>
                                                                                </div>
                                                                                {(breakdown.tienDien || 0) > 0 && (
                                                                                    <div className="flex items-center text-yellow-600">
                                                                                        <Zap className="h-3 w-3 mr-1" />
                                                                                        <span>Điện: {formatCurrency(breakdown.tienDien)}</span>
                                                                                    </div>
                                                                                )}
                                                                                {(breakdown.tienNuoc || 0) > 0 && (
                                                                                    <div className="flex items-center text-cyan-600">
                                                                                        <Droplets className="h-3 w-3 mr-1" />
                                                                                        <span>Nước: {formatCurrency(breakdown.tienNuoc)}</span>
                                                                                    </div>
                                                                                )}
                                                                                {(breakdown.tienDichVu || 0) > 0 && (
                                                                                    <div className="flex items-center text-purple-600">
                                                                                        <Wrench className="h-3 w-3 mr-1" />
                                                                                        <span>DV: {formatCurrency(breakdown.tienDichVu)}</span>
                                                                                    </div>
                                                                                )}
                                                                            </div>
                                                                        )}
